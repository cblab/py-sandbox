...
def evaluate_heuristic(heuristic):
    '''
    A function that tells me how accurate the heuristic is
    '''
    ...
    predictions = heuristic(file_path)

    # check the accuracy
    correct = pd.DataFrame(data=predictions.values(), index=predictions.keys(), columns=['Survived'])
    print('{:.3f}% correct'.format(
        100 * (correct == titanic[['PassengerId', 'Survived']].set_index('PassengerId')).values.sum() / len(correct)))
...
evaluate_heuristic(simple_heuristic)
